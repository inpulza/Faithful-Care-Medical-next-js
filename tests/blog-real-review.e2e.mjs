...
try{
 ...
 for(const post of posts){
  ...
  const preview=page.locator(".saved-article-preview");await preview.getByRole("heading",{level:1}).waitFor();assert.equal(await preview.locator("img:not(.blog-author-avatar img)").count(),3);assert.equal(await preview.locator("script,iframe,object").count(),0);
