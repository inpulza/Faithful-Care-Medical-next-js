...
try{
 ...
 for(const [width,height] of sizes){await page.setViewportSize({width,height});await page.locator('iframe[title="Private article preview"]').evaluate(el=>scrollTo({top:el.getBoundingClientRect().top+scrollY-24,behavior:"instant"}));await page.frameLocator('iframe[title="Private article preview"]').getByRole("heading",{level:1}).waitFor({state:"visible"});await page.waitForTimeout(350);await page.screenshot({path:"artifacts/blog-real/article-"+post.language+"-"+width+"x"+height+".png"});}
