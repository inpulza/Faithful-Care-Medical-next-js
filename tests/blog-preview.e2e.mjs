...
try{
 ...
 await page.getByRole("button",{name:"SEO & links",exact:true}).click();await page.getByLabel("URL slug",{exact:true}).fill(slug);
 ...
 const config=await context.request.get(base+"/api/admin/blog/auto/config");assert.equal(config.status(),200);
 ...
 await fs.writeFile("artifacts/blog-preview/result.json",JSON.stringify({sha:expected,base,checkedAt:new Date().toISOString(),status:"passed",checks:["protected login/logout","private database draft","public draft 404","real source audit","AI configuration read without generation; Google disabled","actual blob upload and review","sanitized preview","five viewports"],privateQaSlug:slug},null,2));
