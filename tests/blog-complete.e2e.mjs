...
try{
 for(const [width,height] of [[1440,900],[390,844]]){
  ...
  await page.getByLabel(/^SEO title/).fill("Prepare for a primary care visit | Faithful Care");
  await page.getByLabel(/^SEO description/).fill("A local verification fixture for the Faithful Care editorial workflow, including sources, image review and the protected publication process.");
  await page.getByLabel("Tags (comma separated)",{exact:true}).fill("prevention, visit preparation");
  await page.getByText("Edit source references",{exact:true}).click();
  await page.getByLabel("Medical sources (one URL per line)",{exact:true}).fill(source);
  await page.getByRole("button",{name:"Edit text",exact:true}).click();
  await page.getByText("Author and disclaimer",{exact:true}).click();
  ...
  await page.getByRole("button",{name:"Upload candidate",exact:true}).click();
  await page.getByRole("status").filter({hasText:"Candidate uploaded"}).waitFor();
  await page.locator(".media-inline").getByRole("button",{name:"Use this image",exact:true}).first().click();await page.getByRole("status").filter({hasText:"Image selected"}).waitFor();
  ...
  await page.getByRole("button",{name:"Unpublish",exact:true}).click();await page.getByRole("status").filter({hasText:"Returned to draft. The article is private."}).waitFor();
  assert.equal((await context.request.get(config.baseUrl+"/blog/"+slug)).status(),404);
