...
export default function Sources({api}:{api:(path:string,method?:string,body?:unknown)=>Promise<any>}){
 ...
 return <section className="source-workspace" aria-labelledby="sources-title">
  ...
  <div className="source-panel">
   ...
   <div id="source-view">
   {tab==="library"?<>
    <div className="source-toolbar"><div className="dashboard-filters"><label>Search sources<input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Topic, source or URL"/></label><label>Source status<select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All sources</option><option value="approved">Approved</option><option value="needs-review">Needs approval</option><option value="unhealthy">Needs a health check</option></select></label></div><p className="source-result-count" role="status">{visible.length} of {sources.length} sources</p></div>
    <div className="source-table-head" aria-hidden="true"><span>Source</span><span>Health & approval</span><span>Quality</span><span>Usage</span><span>Actions</span></div>
    <div className="source-list">{visible.map(s=><article key={s.url} className="source-card" aria-label={s.title}>
     <div className="source-row">
      ...
      <div className="source-state"><span className={"status-badge "+(s.health==="healthy"?"status-published":"status-pending_review")}>{s.health}</span><span className={"source-approval "+(s.approved?"is-approved":"")}>{s.approved?"Approved to cite":"Needs approval"}</span><small title={date(s.checked_at)}>{s.checked_at?"Checked "+shortDate(s.checked_at):"Not checked yet"}{s.http_status?" · "+s.http_status:""}</small></div>
      ...
      <div className="source-actions"><button disabled={busy} className="secondary" onClick={()=>act(async()=>{await api("links/check","POST",{url:s.url});})}>Check live URL</button><button disabled={busy||(!s.approved&&s.health!=="healthy")} className="secondary" onClick={()=>act(async()=>{await api("links/approve","POST",{url:s.url,approved:!s.approved});})}>{s.approved?"Block source":"Approve source"}</button></div>
     </div>
   ...
   </>:<div className="source-history"><div className="source-history-intro"><h3>Consultation history</h3><p>Latest 100 checks, research consultations and approval decisions. Research copies last 24 hours; manual health checks always fetch the live page.</p></div>{history.length?<><div className="source-history-head" aria-hidden="true"><span>Activity</span><span>Source & result</span><span>When</span></div>{history.map(h=><article key={h.id}><span className="status-badge">{h.action.replaceAll("_"," ")}</span><div><p>{h.detail.url||"Source library"}</p>{h.detail.health&&<small>{h.detail.health} · Quality {h.detail.score}/100</small>}</div><time dateTime={h.created_at}>{date(h.created_at)}</time></article>)}</>:<p className="dashboard-empty">No source activity recorded yet.</p>}</div>}
   </div>
   <p className="source-footnote">Link health and source quality are separate from clinical review. Review medical claims before publishing.</p>
  </div>
 </section>;
}
