...
import {sanitize,wordCount} from "../../server/blog/content";
...
export async function articleMetadata(language:Language,slug:string):Promise<Metadata>{
 ...
 openGraph:{type:"article",title:post.data.metaTitle||post.title,description:post.data.excerpt,url:DOMAIN+path(post),...(post.data.hero?{images:[{url:post.data.hero,alt:post.data.heroAlt}]}:{})},
...
export async function ArticlePage({language,slug}:{language:Language;slug:string}){
 ...
 publisher:{"@type":"Organization",name:"Faithful Care Medical Services",url:DOMAIN},...(post.data.hero?{image:post.data.hero}:{})};
 return <BlogChrome><main className="blog-article"><Link href={(es?"/es":"")+"/blog"} className="blog-eyebrow">{es?"← Diario de salud":"← Health & Care Journal"}</Link><h1>{post.title}</h1><p className="intro">{post.data.excerpt}</p><div className="blog-meta"><span>{post.data.author}</span>{post.data.reviewConfirmed&&<span>{es?"Revisión clínica":"Clinical review"}: {post.data.reviewer}</span>}<span>{new Date(post.published_at!).toLocaleDateString(es?"es-US":"en-US",{year:"numeric",month:"long",day:"numeric",timeZone:"UTC"})}</span></div>{other&&<Link className="blog-translation" href={path(other)} hrefLang={other.language}>{es?"Read in English":"Leer en español"}</Link>}{post.data.hero&&<img src={post.data.hero} alt={post.data.heroAlt}/>}<div className="blog-copy" dangerouslySetInnerHTML={{__html:sanitize(post.content)}}/><aside className="blog-disclaimer">{post.data.disclaimer}</aside><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema).replace(/</g,"\\u003c")}}/></main></BlogChrome>;
}
