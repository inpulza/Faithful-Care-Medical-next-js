...
export function aiConfig(){
 ...
 return {key:process.env.OPENAI_API_KEY,model:process.env.BLOG_AI_MODEL||"gpt-4o-mini"};
}
export async function generateJson(instruction:string,data:unknown){
 ...
 const response=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+config.key},
  signal:AbortSignal.timeout(150000),body:JSON.stringify({model:config.model,messages:[{role:"system",content:instruction},{role:"user",content:JSON.stringify(data)}],response_format:{type:"json_object"},max_tokens:7500})});
