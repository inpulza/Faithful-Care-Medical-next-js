...
export async function generateJson(instruction:string,data:unknown,images:string[]=[],schema?:ZodTypeAny,timeoutMs=120000){
 ...
 const response=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+config.key},
  signal:AbortSignal.timeout(timeoutMs),body:JSON.stringify({model:config.model,messages:[{role:"system",content:instruction},{role:"user",content:images.length?[{type:"text",text:JSON.stringify(data)},...images.map(url=>({type:"image_url",image_url:{url,detail:"low"}}))]:JSON.stringify(data)}],response_format:schema?{type:"json_schema",json_schema:{name:"editorial_result",strict:true,schema:providerSchema(schema)}}:{type:"json_object"},max_completion_tokens:12000,...(config.model.startsWith("gpt-5")?{reasoning_effort:"low"}:{})})});
...
export function providerSchema(schema:ZodTypeAny){
 const converted=zodToJsonSchema(schema,{$refStrategy:"none",postProcess:(value)=>{if(value&&typeof value==="object"&&"format" in value&&value.format==="uri")delete value.format;return value;}});
 const {$schema,...result}=converted;return result;
}
