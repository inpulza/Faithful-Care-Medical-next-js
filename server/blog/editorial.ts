...
export type Brief={audience:string;intent:string;sections:string[];facts:{claim:string;url:string;support:string}[];limits:string[]};
...
export function assemble(candidate:Candidate,language:Language,article:Article,metadata:Metadata){
 const content=clean(article.content),allowed=[...internalLinks(candidate.category,language),...candidate.sourceUrls];
 assertLinks(content+" "+article.title+" "+JSON.stringify(metadata),allowed);
